...
bool gz::sim::systems::ArduPilotPlugin::InitSockets(sdf::ElementPtr _sdf) const
{
    ...
    const char * inetAddr = this->dataPtr->fdm_address.c_str();
    char addrstr[100];
    void *ptr;
    ...
    if (ret != 0)
    {
        gzwarn << "fdm_address: " 
              << this->dataPtr->fdm_address 
              << ", could not resolve to ip.\n";  
    }
    else
    {
        ptr = &((struct sockaddr_in *) addrs->ai_addr)->sin_addr;
        this->dataPtr->fdm_address = std::string(inet_ntop(addrs->ai_family, ptr, addrstr, 100));
    }
...
)
{
    ...
    // gzmsg << "Paused: " << _info.paused 
    //   << " sim time: " << std::chrono::duration_cast<std::chrono::duration<double> >(_info.simTime).count()
    //   << " last update: " << std::chrono::duration_cast<std::chrono::duration<double> >(this->dataPtr->lastControllerUpdateTime).count()
    //   << "\n";
