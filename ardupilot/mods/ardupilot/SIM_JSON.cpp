...
void JSON::set_interface_ports(const char* address, const int port_in, const int port_out)
{
    ...
    if (ret != 0)
    {
        printf("Could not resolve %s into ip address\n",address);
    }
    else
    {
        ptr = &((struct sockaddr_in *) addrs->ai_addr)->sin_addr;
        const char* new_address = inet_ntop(addrs->ai_family, ptr, ip_string, 100);
        if (strcmp("127.0.0.1",new_address) != 0) {
            target_ip = new_address;
        }
    }
